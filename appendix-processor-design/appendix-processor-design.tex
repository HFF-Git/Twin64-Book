...
The instruction decoder interprets the two instructions contained in an instruction bundle. The processor supports the parallel execution of different instruction classes, including Compute, Branch, Memory, and System instructions. Unlike the single issue architecture, where each instruction occupies one instruction word, the dual issue architecture fetches and decodes instruction bundles. A bundle contains two instructions together with a bundle identifier that specifies the valid instruction pairing.
...
The two instructions are encoded using the standard Twin-64 instruction format. However, the instruction group field is omitted, as the instruction class is encoded by the bundle identifier. The bundle identifier therefore determines the permitted combination of instruction types.
...
Other combinations are not encoded as instruction bundles and must be issued as separate instructions. Restricting the available bundle types simplifies instruction decoding, hazard detection, resource allocation, and pipeline control while still capturing the instruction combinations most frequently encountered in practice.
