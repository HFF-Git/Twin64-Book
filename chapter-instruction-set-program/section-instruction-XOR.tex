...
%--------------------------------------------------------------------------------
\pagebreak
\unnumberedSection{XOR - Boolean Operation}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Syntax}
  \texttt{XOR[.N] RegR, RegB, RegA} \\
  \texttt{XOR[.N] RegR, RegB, val}
  
  \texttt{XOR[.N/D/W/H/B] RegR, ofs( RegB )} \\
  \texttt{XOR[.N/D/W/H/B] RegR, RegX ( RegB )}
\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Format}
  The XOR instruction uses the register, the following instruction formats. \\
	
  \begin{flushleft}	
  \drawInstrFormat
  	{0,1,3,5,5.5,6,6.5,8.5,9.5,11.5,16}
  	{\OpCodeGrpALU,\OpCodeXOR, RegR, N, 0, 0, RegB, 0, RegA, 0}
	
  \drawInstrFormat
  	{0,1,3,5,5.5,6,6.5,8.5,16}
  	{\OpCodeGrpALU,\OpCodeXOR, RegR, N, 0, 1, RegB, val}

  \drawInstrFormat
  	{0,1,3,5,5.5,6,6.5,8.5,9.5,16}
  	{\OpCodeGrpMEM,\OpCodeXOR, RegR, N, 0, 0, RegB, dw, ofs}
 
  \drawInstrFormat
  	{0,1,3,5,5.5,6,6.5,8.5,9.5,11.5,16}
  	{\OpCodeGrpMEM,\OpCodeXOR, RegR, N, 0, 1, RegB, dw, RegX, 0}
  \end{flushleft}
\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Description}
  The \texttt{XOR} instruction performs a bitwise XOR operation on two 64-bit operands and stores the result in the target register \texttt{RegR}. The result can be negated by setting the "N" bit. This instruction supports the immediate, register, and both memory-reference formats. The boolean operation itself does not trigger any traps, but the indexed formats may cause a memory-reference–related trap.
\end{iPageDescEntry}
