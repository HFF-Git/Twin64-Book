...
The most significant bit (\texttt{cond[2]}) acts as an \emph{invert bit}: flipping it inverts the sense of the comparison.  
The relationships between base and inverted conditions are as follows:
...
All conditions used by the comparator and branch unit are derived from simple signals computed by the ALU after a subtraction, or directly from the result when testing against zero. The example below illustrates a possible Verilog implementation:
...
All comparisons are signed by default; unsigned comparisons can be implemented by treating operands as unsigned integers or through a future \texttt{CMPU} variant if required.  
The \texttt{EV}/\texttt{OD} conditions allow efficient branching on bit~0, which is useful for address alignment tests, distinguishing even/odd indices, and loop unrolling.
