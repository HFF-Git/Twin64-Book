...
\begin{itemize}
    ...
    A write issued through the broadcast module HPA address updates the corresponding register in \emph{every} module on the bus. Broadcast writes are useful for system-wide resets, synchronization operations, or events that must be delivered simultaneously to all modules.
\end{itemize}
...
\begin{tikzpicture}[scale=0.7, transform shape]
	...
	\node[left] at (\leftofs, 7 - 0.2) {\texttt{0x100}};
...
IO\_COMMAND for resetting

IO\_ADDRESS  ( read from HAPOA page )

IO\_DATA     ( data read )

IO\_STATUS for module status / health

IO\_SPA for finding the SPA address range
...
IO\_MODULE\_CAPABILITIES

IO\_ENTRY\_POINT\_TAB\_OFS

IO\_ENTRY\_POINT\_TAB\_LEN
...
I/O module 63, i.e. the last page in the HPA address range, is the broadcast module. It contains no actual functions. Writes issued to broadcastable registers within this range are not directed to a single module. Instead, the write operation is applied simultaneously to all I/O modules on the system bus. This mechanism is typically used for system-wide commands such as resets, synchronization events, or configuration updates that must reach every module at once.
