...
\begin{tikzpicture}[scale=0.9, transform shape]
	...
	\draw[thick, dashed] (\leftofs, 3.5) -- (\rightofs, 3.5);
  	\node at ( \middleofs, 4 ) { Parameter Area register};
	  	
  	% Stack Marker
  	\draw[thick] (\leftofs, 2) -- (\rightofs, 2);
  	\node at ({\leftofs - 1}, {0}) { SP };
  	\node at ( \middleofs, {1}) { Stack Marker };
...
\resizebox{0.7\linewidth}{!}{
    ...
    % Grid and axes
    \draw[step=1cm, gray!30, thin] (0,0) grid (24,32);
    ...
    % Example content
    \draw[red, thick] (2,2) rectangle (14,30);
    \node[blue] at (8,16) {\large Center text};
	\end{tikzpicture}
}
\end{center}
\label{fig:Stack Frame}
\caption{A 16×32 coordinate system scaled to fit the page.}
\end{figure}
