...
\begin{iPageDescEntry}{Description}
  The \texttt{AND} instruction performs a bitwise AND operation on two 64-bit operands and stores the result in the target register \texttt{RegR}. The second operand, \texttt{RegB}, can be negated by setting the "C" bit, and the result can be negated by setting the "N" bit. This instruction supports the immediate, register, and both memory-reference formats. The Boolean operation itself does not trigger any traps, but the indexed formats may cause a memory-reference–related trap.
\end{iPageDescEntry}
