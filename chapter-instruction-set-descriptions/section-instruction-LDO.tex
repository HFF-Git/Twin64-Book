...
\texttt{LDO [.B/H/W/D] RegR, RegX(RegB)}
\end{iPageDescEntry}
...
The \texttt{LDO} instruction loads an offset into a general register. Two formats are available: the immediate-offset format and the register-indexed format. In the immediate-offset format, the signed immediate offset is shifted left by the \texttt{dw} field of the instruction and added to \texttt{RegB} to form the effective address. The default for the shift amount is 1 ( .B ).


In the register-indexed format, \texttt{RegX} is added to \texttt{RegB} to form the effective address. The computed value is then stored in \texttt{RegR}.
...
??? decide if whether we drop scaled offset here or not...
