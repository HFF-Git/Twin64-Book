...
\begin{iPageDescEntry}{Format}
    The \texttt{PRB} instruction uses both the register format and the special-9 format. \\
...
\begin{iPageDescEntry}{Description}
    ...
    The result of the probe is returned in \texttt{RegR}:  
    \texttt{0} indicates that no mapping exists, while \texttt{1} indicates a valid mapping.  

    In the register format, the access mode is specified in \texttt{RegA}.  
    In the special-9 format, the mode is encoded directly in the instruction.  
