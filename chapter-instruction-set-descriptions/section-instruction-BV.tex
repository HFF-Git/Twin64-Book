%--------------------------------------------------------------------------------
\pagebreak
\unnumberedSection{BV - Branch Vectored}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Syntax}
\texttt{BV [ RegX ] (  RegB ) [ , RegR ]}
\end{iPageDescEntry}
...
\drawInstrFormat
    ...
    {\OpCodeGrpBR,\OpCodeBV, RegR, 0, RegB, 0, RegX, 0}
\end{flushleft}
\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Description}
he \texttt{BV} instruction performs an unconditional region relative branch. The target address offset is formed by taking the contents of \texttt{RegX}, left shifting by two, and adding the result to \texttt{RegB}. The target address region is  the region portion of the current instruction address.
