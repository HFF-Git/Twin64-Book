...
\begin{iPageDescEntry}{Format}
    ...
    \drawInstrFormat
        {0,1,3,5,6.5, 16}
        {\OpCodeGrpSYS, \OpCodeRFI, 0, 0, 0 }
    \end{flushleft}
\end{iPageDescEntry}

\begin{iPageDescEntry}{Description}
    The \texttt{RFI} instruction restores processor state after an interrupt
    or exception handler has completed. Execution resumes at the program counter saved during the interrupt and the processor state (including privilege level, interrupt enable flags, and other PSR fields) is restored from the saved copy.
\end{iPageDescEntry}
