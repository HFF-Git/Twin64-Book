%--------------------------------------------------------------------------------
%
%
%--------------------------------------------------------------------------------
\pagebreak
\section*{XOR Boolean Operation}
\addcontentsline{toc}{section}{XOR Boolean Operation}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Syntax}
  \texttt{XOR[.C/N] RegR, RegB, RegA} \\
  \texttt{XOR[.C/N] RegR, RegB, val}
  
  \texttt{XOR[.C/N/D/W/H/B] RegR, ofs( RegB )} \\
  \texttt{XOR[.C/N/D/W/H/B] RegR, RegX ( RegB )}
\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Format}
  The XOR instruction uses the register, the immediate-19, the indexed and the register indexed instruction formats. \\
	
  \begin{flushleft}	
  \drawInstrFormat
  	{0,1,3,5,5.5,6,6.5,8.5,9.5,11.5,16}
  	{\OpCodeGrpALU,\OpCodeXOR, RegR, N, C, 0, RegB, 0, RegA, 0}
	
  \drawInstrFormat
  	{0,1,3,5,5.5,6,6.5,8.5,16}
  	{\OpCodeGrpALU,\OpCodeXOR, RegR, N, C, 1, RegB, val}

  \drawInstrFormat
  	{0,1,3,5,5.5,6,6.5,8.5,9.5,16}
  	{\OpCodeGrpMEM,\OpCodeXOR, RegR, N, C, 0, RegB, dw, ofs}
 
  \drawInstrFormat
  	{0,1,3,5,5.5,6,6.5,8.5,9.5,11.5,16}
  	{\OpCodeGrpMEM,\OpCodeXOR, RegR, N, C, 1, RegB, dw, RegX, 0}
  \end{flushleft}
\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Description}
  The \texttt{XOR} instruction performs a bitwise XOR operation on two 64-bit operands and stores the result in the target register \texttt{RegR}. The result can be negated by setting the "N" bit. This instruction supports the immediate, register, and both memory-reference formats. The Boolean operation itself does not trigger any traps, but the indexed formats may cause a memory-reference–related trap.
\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescOpEntry}{Operation}
\begin{lstlisting}[style=iPageOpStyle]
if ( instr.[C] ) RegB <- not( RegB );
RegR <- RegB ^ RegA;           (register format)
RegR <- RegB ^ immOperand();   (immediate format)
RegR <- RegR ^ memOperand();   (indexed formats)
if ( instr.[N] ) RegR <- not( RegR );
\end{lstlisting}
\end{iPageDescOpEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Exceptions}
  Data Alignment Trap\\
  Memory Access Violation Trap\\
  Memory Protection Violation Trap\\
  Data TLB Miss Trap
\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Notes}
  None. 
\end{iPageDescEntry}

 