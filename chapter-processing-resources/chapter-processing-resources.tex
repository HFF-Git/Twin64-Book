...
\begin{tikzpicture}[scale=0.9, transform shape]
	...
	\draw[thick] (\leftofs, 15) -- (\rightofs, 15);
  	...
  	\node at ( \middleofs + 3, {7 + 0.5}) { Interrupt Vector Adress };
  	
  	% CR9
  	\draw[thick] (\leftofs, 6) -- (\rightofs, 6);
  	\node at ({\leftofs - 0.75}, {6 + 0.5}) { CR9 };
  	\node at ( \middleofs, {6 + 0.5}) { External Interrupt Request Register };
  	
  	% CR10
  	\draw[thick] (\leftofs, 5) -- (\rightofs, 5);
  	\node at ({\leftofs - 0.75}, {5 + 0.5}) { CR10 };
  	\node at ( \middleofs, {5 + 0.5}) { External Interrupt Mask };
  	
  	% CR11
  	\draw[thick] (\leftofs, 4) -- (\rightofs, 4);
  	\node at ({\leftofs - 0.75}, {4 + 0.5}) { CR11 };
  	\node at ( \middleofs, {4 + 0.5}) { Interruption PSW };
  	
  	% CR12
  	\draw[thick] (\leftofs, 3) -- (\rightofs, 3);
  	\draw[thick] (\middleofs, 3) -- (\middleofs, 4);
  	\node at ({\leftofs - 0.75}, {3 + 0.5}) { CR12 };
  	\node at ( \leftofs + 3, {3 + 0.5}) { 0 };
  	\node at ( \rightofs - 3, {3 + 0.5}) { Interruption Instruction };
  	
  	% CR13
  	\draw[thick] (\leftofs, 2) -- (\rightofs, 2);
  	\node at ({\leftofs - 0.75}, {2 + 0.5}) { CR13 };
  	\node at ( \middleofs, {2 + 0.5}) { Interruption Address };

  	% CR14
  	\draw[thick] (\leftofs, 1) -- (\rightofs, 1);;
  	\node at ({\leftofs - 0.75}, {1 + 0.5}) { CR14 };
  	\node at ( \middleofs, {1 + 0.5}) { Scratch Reg 0 };
  	
  	% CR15
  	\draw[thick] (\leftofs, 0) -- (\rightofs, 0);
  	\node at ({\leftofs - 0.75}, {0 + 0.5}) { CR15 };
  	\node at ( \middleofs, {0 + 0.5}) { Scratch Reg 1 };
...
\subsection*{External Interrupt Request Register}
\addcontentsline{toc}{subsection}{External Interrupt Request Register}

I/O module sets a bit for service.

\subsection*{External Interrupt Mask}
\addcontentsline{toc}{subsection}{External Interrupt Mask}

processor can mask of a position.

\subsection*{Interruption PSW and instruction}
\addcontentsline{toc}{subsection}{Interruption PSW}

interrupt instruction address and instruction.

\subsection*{Interruption Argument}
\addcontentsline{toc}{subsection}{Interruption Address}

additional data what happened.

\subsection*{Scratch Registers}
\addcontentsline{toc}{subsection}{External Interrupt Register}

privileged ? split one each priv level ?
...
\begin{longtable}{@{}p{0.25\linewidth}p{0.6\linewidth}@{}}
    ...
    \midrule
    \textbf{S} & When set, the processor does protection Id checking for a virtual address. \\
    \midrule 
    \textbf{E} & When set, external interrupts are enabled. \\
    \midrule 
    \textbf{V} & When set, single step debug support enabled. \\
    \midrule 
    \textbf{P} & When set, data access is trapped.... \\
    \midrule 
    \textbf{...} & ... \\
\end{longtable}
