%--------------------------------------------------------------------------------
%
%
%--------------------------------------------------------------------------------
\pagebreak
\unnumberedSection{PTLB - Purge TLB entry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Syntax}
\texttt{ptlb [ <vAdr> | ALL ]}
\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Description}

The \texttt{PDTLB} command invalidates entries in the unified Translation Lookaside Buffer (TLB). If a virtual address \texttt{vAdr} is specified, only the TLB entry matching that virtual address range is purged. The \texttt{ALL} parameter will remove all entries from the TLB. The virtual address parameter will remove all TLB entries covering this address. 

\end{iPageDescEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescOpEntry}{Example}
\begin{lstlisting}[style=iPageOpStyle]

(3)-># purge the entire instruction TLB
(3)->ptlb

(4)-># purge the instruction TLB entry covering 0x200_1000_0000
(4)->ptlb 0x200_1000_0000

\end{lstlisting}
\end{iPageDescOpEntry}

%--------------------------------------------------------------------------------
\begin{iPageDescEntry}{Notes}
If the simulator environment consists of serval processors, all TLBs will invalidate the specified virtual address range.
\end{iPageDescEntry}
